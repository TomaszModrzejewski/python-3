...
def get_tf(file_name):
    ...
    return {key : (value, value / total) for (key, value) in word2count.items()}
...
def get_IDF(total_TF):
    ...
    IDF = {keyword: math.log(len(total_TF)/IDF[keyword], 2) for keyword in IDF}
    return IDF

def get_weight(TF, IDF):
    return {key:TF[key][1]*IDF[key] for key in TF}
...
if __name__ == '__main__':
    ...
    total_TF = {
        file_name: get_tf(path + os.sep + file_name)
